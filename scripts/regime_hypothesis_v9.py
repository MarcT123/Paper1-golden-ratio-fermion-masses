#!/usr/bin/env python3
"""
Regime Hypothesis — Standalone Verification
=============================================
...
Formalizes and tests the regime hypothesis H_REG (Sec 4):
Φ-relations manifest in hadronic observables
if and only if R(H) = E_bind/M > R_crit ≈ 0.5.

Contains:
  1. Fisher Exact Test (2×2 table)
  2. Mann-Whitney U Test
  3. ROC analysis for R_crit
  4. Chiral derivation (Σ_s − Σ_ud)
  5. Predictions for exotic hadrons (Sec 6)

Author: M. Trömel, Independent Researcher, Nürtingen, Germany
Date: March 2026
"""
...
# phi_works: True if empirically verified (Sec 4.3)
HADRONS = [
    # QCD-dominiertes Regime (leichte Quarks)
    ("Ω⁻ (sss)",       1672.45, [93.4, 93.4, 93.4],    True),   # Sec 4.3(i): s×Φ⁶
    ("Δ(1232) (uud)",   1232.0,  [2.16, 2.16, 4.67],    True),   # Sec 4.3(ii): decuplet
    ("Σ*(1385) (uus)",  1383.7,  [2.16, 2.16, 93.4],    True),   # Sec 4.3(ii): decuplet
    ("Ξ*(1532) (uss)",  1531.8,  [2.16, 93.4, 93.4],    True),   # Sec 4.3(ii): decuplet
    # Quarkmasse-dominiertes Regime (schwere Quarks)
    ("Ωccc (ccc)",      4761.0,  [1270., 1270., 1270.],  False),  # Sec 5.3: 30% error
    ("Ωbbb (bbb)",     14371.0,  [4180., 4180., 4180.],  False),  # Sec 5.3: 15% error
    ("Λc (udc)",        2286.46, [2.16, 4.67, 1270.],    False),  # Sec 5.3: 58% error
    ("Λb (udb)",        5619.60, [2.16, 4.67, 4180.],    False),  # Sec 5.3
    ("Ωb⁻ (ssb)",       6046.0,  [93.4, 93.4, 4180.],   False),  # Sec 5.3
]
...
def test_predictions():
    ...
    exotics = [
        ...
        ("a₀(980)", 980, [2.16, 4.67], "Φ erwartet (R ≈ 0,99)"),
        ("Tcc(3875)", 3875, [1270., 1270., 2.16, 4.67], "Kein Φ (R ≈ 0,34)"),
        ("X(3872)", 3872, [1270., 1270., 2.16, 2.16], "Kein Φ (R ≈ 0,34)"),
        ("Pc(4312)", 4312, [2.16, 2.16, 4.67, 1270., 1270.], "Kein Φ (R ≈ 0,41)"),
        ("Pc(4440)", 4440, [2.16, 2.16, 4.67, 1270., 1270.], "Kein Φ (R ≈ 0,43)"),
    ]
    ...
    print(f"\n  ANMERKUNG: Die Pentaquark-Zustände Pc haben R ≈ 0,4")
    print(f"  (uudcc̄, 5 Quarks). Dies liegt im Quark-dominierten Regime.")
    print(f"  Wenn als molekulare Zustände (Σc D̄) interpretiert, ist R")
    print(f"  nicht direkt anwendbar — die Vorhersage hängt dann vom")
    print(f"  Bindungsmodell ab.")
...
def main():
    print("="*68)
    print("  REGIME HYPOTHESIS — Complete Verification")
