...
Outputs: fig1_pred_vs_obs.pdf, fig2_regime.pdf, fig3_universality.pdf
"""
import os
import numpy as np
import matplotlib
...
ax1.plot([0.3, 3e5], [0.3, 3e5], 'k--', lw=0.8, alpha=0.5, label='$M_\\mathrm{pred} = M_\\mathrm{PDG}$')

# 5% error band
xx = np.logspace(-0.5, 5.5, 100)
ax1.fill_between(xx, xx*0.95, xx*1.05, alpha=0.08, color='blue', label='$\\pm 5\\%$ band')

# Data points
ax1.scatter(pdg_vals, pred_vals, s=60, c='#1F3864', zorder=5, edgecolors='white', linewidths=0.5)
...
for i, (name, x, y) in enumerate(zip(labels, pdg_vals, pred_vals)):
    ...
    ax1.annotate(name, (x, y), textcoords="offset points", xytext=offset, fontsize=10)
...
ax1.set_xlim(0.3, 3e5)
ax1.set_ylim(0.3, 3e5)
ax1.set_aspect('equal')
ax1.grid(True, alpha=0.2, which='both')

fig1.tight_layout()
fig1.savefig(os.path.join(OUTDIR, 'fig1_pred_vs_obs.pdf'), bbox_inches='tight', dpi=300)
print("Figure 1 saved.")

# ========================================
# FIGURE 2: Regime hypothesis
# ========================================
...
for name, M, sumq, phi in hadrons:
    ...
    ms = 10 if phi else 9
    lw = 1 if phi else 2
    ax2.plot(R, 0.5, marker, color=color, markersize=ms, markeredgewidth=lw)
    yoff = 0.08 if phi else -0.08
    ha = 'center'
    fs = 8.5
    ax2.annotate(name, (R, 0.5+yoff), fontsize=fs, ha=ha, va='center')

# Separation line
ax2.axvline(x=0.5, color='gray', ls='--', lw=1.2, alpha=0.7)
ax2.annotate('$R_{\\mathrm{crit}} = 0.5$', (0.5, 0.85), fontsize=10, 
             ha='center', color='gray')

# Labels for regions
ax2.text(0.25, 0.25, 'Quark-dominated\n(no $\\Phi$)', fontsize=10, 
         ha='center', color='#C00000', style='italic')
ax2.text(0.80, 0.25, 'QCD-dominated\n($\\Phi$ confirmed)', fontsize=10, 
         ha='center', color='#1F3864', style='italic')

# Arrow showing the gap
ax2.annotate('', xy=(0.83, 0.65), xytext=(0.44, 0.65),
            arrowprops=dict(arrowstyle='<->', color='#666666', lw=1.5))
ax2.text(0.635, 0.69, '$\\Delta R = 0.39$', fontsize=9, ha='center', color='#666666')

ax2.set_xlabel('QCD dominance parameter $R = E_\\mathrm{bind}/M$', fontsize=12)
ax2.set_xlim(0.05, 1.05)
ax2.set_ylim(0.1, 0.95)
ax2.set_yticks([])
ax2.set_title('Regime hypothesis: perfect separation ($p = 0.008$)', fontsize=12, pad=10)
ax2.grid(True, axis='x', alpha=0.2)

fig2.tight_layout()
fig2.savefig(os.path.join(OUTDIR, 'fig2_regime.pdf'), bbox_inches='tight', dpi=300)
print("Figure 2 saved.")

# ========================================
# FIGURE 3: Phi-universality (K-scan)
# ========================================
...
ax3.plot(K_vals, max_deltas, 'k-', lw=0.6, alpha=0.6)
ax3.axhline(y=0.080, color='#C00000', ls='--', lw=1, alpha=0.7, label='$\\Phi$ threshold (0.080)')
ax3.axvline(x=PHI, color='#1F3864', ls='-', lw=2, alpha=0.8, label=f'$\\Phi = {PHI:.3f}$')

# Mark Phi
ax3.plot(PHI, 0.080, 'o', color='#1F3864', markersize=8, zorder=5)

ax3.set_xlabel('Constant $K$', fontsize=12)
ax3.set_ylabel('$\\max|\\Delta n|(K)$', fontsize=12)
ax3.set_title('$\\Phi$-universality: no alternative $K$ matches all 5 relations', fontsize=11, pad=8)
...
fig3.tight_layout()
fig3.savefig(os.path.join(OUTDIR, 'fig3_universality.pdf'), bbox_inches='tight', dpi=300)
