...
\author{%
  M.\ Tr\"omel\thanks{Corresponding author. E-mail: marc.troe@gmail.com.
...
of elementary fermions. Starting from the electron mass as the sole seed
of a recursive bootstrap algorithm, 11 Standard Model particle
masses are reproduced using five $\Phi$-based operators, with a mean error of 2.5\%.
The operator exponents $\{3, 4, 11\}$ are Lucas numbers ($L_2, L_3, L_5$;
the selection of these three from the infinite Lucas sequence is empirical),
...
The Standard Model of particle physics contains nine Yukawa coupling
constants that determine the masses of the charged fermions
(6 quarks and 3 charged leptons); with massive neutrinos, additional
parameters enter. None of these are predicted by the theory; they are
extracted from experiment and span more than five orders of magnitude,
from $m_e = 0.511$~MeV to $m_t = 172{,}760$~MeV. Whether an organizing
...
Starting from the electron mass $m_e = 0.511$~MeV as the sole seed,
...
geometric mean (impedance) of two already-identified masses.
Each prediction in the table below uses the PDG mass of the
parent particle, not a chain-propagated value; the algorithm
is defined in Appendix~\ref{app:algorithm}.
...
Rows are sorted by mass, not by algorithmic generation order;
see Appendix~\ref{app:algorithm} for the iterative construction.
...
Down     & 4.706  & 4.67  & $m_u \times \Phi^\Phi$       & 0.8\% \\
...
eleven chain entries depend on this convention (those using $u$, $d$,
or $s$ as parent mass); a convention-independent
subset is tested separately in Section~\ref{sec:convindep}.
The bottom quark ($\overline{\text{MS}}$ at $\mu = m_b$) is counted
as convention-independent because the self-scale running mass is
uniquely defined and lies well within the perturbative QCD regime
($m_b \gg \Lambda_{\text{QCD}}$), whereas the light quark masses at
$\mu = 2$~GeV depend on an external choice of renormalization point.
...
\noindent
\textbf{Result:}
$G_\text{obs} = 0.181$ vs.\ $G_\text{min} = 1/6 = 0.167$ (perfect uniformity).
...
$p = 0.019$; a basis scan over 300 random bases $\beta \in [1.05, 3.5]$
gives $\Phi$ rank \#1/301 ($p = 0.003$). The signal exists
...
channel (since $\pi \approx 3.14$ lies near $\Phi^{2.4} \approx 3.17$).
The base~2 requires $\geq 10\%$ tolerance (9/11) and reaches 11/11 at
15\%. The secondary cluster at $\beta \approx 3.17 \approx \Phi^{2.4}$
...
muon) to $\sim 20\%$ (up quark). For 7 of 11 particles, the relative
...
The three light quarks ($u$, $d$, $s$) and charm carry uncertainties
of order 1--20\%.
...
(perturbative regime).
Note: $R$ is a heuristic classification parameter, not a
scheme-independent observable; the decomposition
$M_H = \sum m_i + E_\text{bind}$ depends on the
$\overline{\text{MS}}$ renormalization scheme.
Its utility lies in its empirical separating power, tested below.
...
$\overline{\text{MS}}$ values. Hadron masses from PDG~2024 unless
noted.}
\label{tab:regime}
...
$\Omega_{ccc}(4761)^\dagger$ & 4{,}761 & 3{,}810 & 0.20 & $\times$ & Quark \\
$\Omega_{bbb}(14371)^\dagger$& 14{,}371 & 12{,}540 & 0.13 & $\times$ & Quark \\
\bottomrule
\multicolumn{6}{@{}l}{\footnotesize $^\dagger$Not yet observed;
  masses from lattice QCD predictions.}
\end{tabular}
\end{table}
...
found unsupported or falsified.

\begin{table}[H]
\centering
\caption{Seven tested hypotheses without support.}
\label{tab:falsified}
\begin{tabular}{@{}lll@{}}
\toprule
...
\noindent
\textbf{What we do claim:} A statistically significant
$\Phi$-structure exists in the elementary
fermion masses, not reproducible by alternative standard mathematical
constants, with a clearly defined, physically motivated domain
of validity.
...
the highest-power operator $\Phi^{11} \approx 199$. The observed mass
ratio $m_t / m_p = 184.1$ deviates from $\Phi^{11} = 199.0$ by 8.1\%;
because the operator is a large multiplier, this fractional mismatch
translates into an absolute error of $\sim 14{,}000$~MeV. This is a
...
exponents $n = 3$ ($L_2$), $n = 11$ ($L_5$), and $n = 17$
($= L_2 + L_5 + L_2$, itself not a Lucas number). The mathematical
structure was \emph{not} chosen to maximize $\Delta n$ statistics.
...
A statistically significant golden ratio structure pervades the
elementary fermion mass spectrum
of the Standard Model~\cite{PDG2024} ($p < 0.001$
combined), not reproducible by alternative standard mathematical
constants ($p = 0.001$).
...
\noindent\emph{Note:}
The best-match rule does not uniquely determine the chain shown in
Table~\ref{tab:genesis}. Alternative paths with comparable errors
exist (e.g., $m_d \times \Phi^{11}$ for the proton at 0.95\%
vs.\ $m_\pi \times \Phi^4$ at 2.0\%). Table~\ref{tab:genesis}
reports the chain selected for physical interpretability
(minimal operator reuse, impedances connecting different sectors).
All statistical tests in Section~\ref{sec:stats} are independent
of this path selection.
...
\begin{itemize}
  \item \texttt{master\_verification\_v10.py} --- All Level~A, B, and D
        tests including the full 167-hadron holdout (1171 lines, Python~3.8+, NumPy, SciPy)
  \item \texttt{regime\_hypothesis\_v9.py} --- Regime hypothesis, ROC
        analysis, chiral derivation (288 lines)
\end{itemize}
...
decuplet splittings: the smallest is
$\Omega^- - \Xi^* \approx 140$~MeV; the largest is
$\Sigma^* - \Delta \approx 152$~MeV. Padding to $[100, 200]$~MeV
adds generous margin on each side.
The quark-mass difference range $[50, 200]$~MeV spans from the
isospin-breaking scale ($m_d - m_u \approx 2.5$~MeV, padded generously)
to twice the strange quark mass ($m_s \approx 93$~MeV, doubled). This ensures
